Attribute VB_Name = "fetchingRankingBlog"
Sub fetchingRankingBlog()
Attribute fetchingRankingBlog.VB_ProcData.VB_Invoke_Func = "B\n14"
    ...
    For r = 1 To selRng.Rows.Count
        ...
        If dict.Exists(dictKey) Then
            wsSrc.Cells(selRng.row + r - 1, "S").value = dict(dictKey)
        Else
            wsSrc.Cells(selRng.row + r - 1, "S").value = ""
        End If
    Next r
    ...
    For r = 1 To selRng.Rows.Count
        wsSrc.Cells(selRow + r - 1, todayCol).value = wsSrc.Cells(selRow + r - 1, "S").value
    Next r
    ...
    For r = 1 To selRng.Rows.Count
        ...
        wsSrc.Cells(selRow + r - 1, "Q").value = firstRank
        wsSrc.Cells(selRow + r - 1, "R").value = highestRank
        wsSrc.Cells(selRow + r - 1, "T").value = duration
    Next r
